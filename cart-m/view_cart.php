					...
					<?php
						...
						$query = $conn->query($sql);
							...
							while($row = $query->fetch_assoc()){
								...
								<tr>
									<td>
										<a href="delete_item.php?id=<?php echo $row['pid']; ?>&index=<?php echo $index; ?>" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></a>
									</td>
									...
									<input type="text" class="form-control" hidden value="<?php echo $row["stock"]; ?>" name="stock_<?php echo $index; ?>">
									<input type="text" class="form-control" hidden value="<?php echo $row["title"]; ?>" name="title_<?php echo $index; ?>">
								</td>
									<td><?php echo number_format($_SESSION['qty_array'][$index]*$row['normal_price'], 2); ?></td>
									<?php $total += $_SESSION['qty_array'][$index]*$row['normal_price']; ?>
								</tr>
								<?php
								$index ++;
							}
